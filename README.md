...
*   **数据实证结论**：
    *   **高波动技术异象弱且不显著（基于 `02_high_minus_low_summary.csv`）**：测试的 8 个策略中，仅 5 个策略的 HML 年化超额收益为正，且在 5% 水平下全线不显著。MA20 策略表现最强（HML 年化收益为 **5.27%**，t 统计量为 **0.91**）。而在长周期策略 MA120 上异象发生反转（HML 年化收益为 **-10.23%**，t 统计量为 **-1.77**）。
    *   **交易活跃度对异象呈现方向性弱削弱（基于 `03_activity_attenuation_summary.csv`）**：24 个活跃度衰减测试中 18 个在方向上表现出 HML 收益随活跃度上升而收窄的特征（无统计显著性）。其中 MA120 long_short 的 HML 年化收益从低活跃度下的 **1.20%** 收缩至高活跃度下的 **-16.44%**（衰减幅度为 **-22.24%**，t = **-1.47**）。
    *   **选择后收益与夏普率系统性衰减（基于 `04_post_selection_decay_summary.csv`）**：IS 筛选出的 Top 10 策略跨区间表现大幅下滑，夏普比率中位数从 IS 阶段的 **0.68** 降至 OOS 阶段的 **0.35**（保留率 55.5%），在 Pseudo-Live 阶段进一步收缩至 **0.11**（保留率仅 **16.6%**），没有策略表现稳健。
...
*   **框架与流程**：
    *   **模型拟合与选择**（`Traditional_Model_Selection_Sugar.ipynb`）：针对 20 个主力商品期货日收益率，拟合包含常数/AR(1) 均值，结合 GARCH, EGARCH11/21, GJR, TARCH 条件方差，以及 Normal, Student-t, Skewed Student-t, GED 误差分布的 19 种参数组合；通过标准化残差 Ljung-Box 自相关（$z$ 与 $z^2$ 的 10/20/30 阶滞后）与 ARCH-LM 检验（10阶）过滤后，以 **BIC 最小化**确立最优模型。
...
*   **数据实证结论**：
    *   **EGARCH 族与厚尾假定占优（基于 `02_best_model_by_symbol.csv`）**：在 20 个商品期货中，有 15 个品种的最优模型为 **EGARCH 族**，且全线排除正态分布。模型表现出极强的波动持续性（以白糖为例，$\beta_1 = 0.9899$）和显著的厚尾特征（白糖自由度 $\nu = 4.497$，沪银 $\nu = 3.164$）。
    *   **数据驱动谱检验精准穿透传统盲区（基于 `13_hong_lee_data_driven_bandwidth_selected-Copy1.csv`）**：在标准化残差均通过传统 Ljung-Box 与 ARCH-LM 检验的前提下，自适应数据驱动谱检验在 5% 显著性水平下**显著拒绝了 3 个最优模型的充分性**：
    ...
    *   **部分品种第四矩理论收敛受限（基于 `10_model_health_summary.csv`）**：检测发现 **AG (沪银)** 和 **CF (棉花)** 等品种的自由度估计值 $\nu \le 4$（AG 为 3.1640，CF 为 3.2287），触发了 `hl_inference_limited_fourth_moment` 警报（理论峰度发散，渐进方差推导受限）。
    *   **横截面条件波动率及仓位参考因子分化严重（基于 `06_current_vol_state_and_position_scaling.csv`）**：最新截面数据日（2026年6月18日）品种分化明显：**AG (沪银)** 处于极端高波状态（条件波动率 **3.805%**，历史分位数 **95.86%**），仓位参考因子缩减至 **0.417x**；而 **AL (沪铝)** 处于常态波动状态（条件波动率 **0.880%**，分位数 **38.71%**），仓位释放至 **1.091x**。
