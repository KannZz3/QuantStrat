...
*   **框架与流程**：
    *   **02_vol_sorted_technical_anomaly.ipynb**：读取日频面板数据，计算各品种信号日 $s$ 的 60 日滚动历史波动率 `rv_60_signal`；横截面每日分为 `Low / Mid / High` 三个波动率桶（要求每日有效标的不少于 9 个）；以 $s$ 日收盘价计算四类技术指标（MA10, MA20, MA60, MA120）的 `long_flat` 和 `long_short` 仓位信号；执行端采用 $t = s + 1$ 的延迟交易机制，以 $t$ 日 Forward 1D 收益率 `fwd_ret_1d`（$t$ 至 $t+1$ 日收盘收益）计算等权策略日收益，并扣除 5 bps 的单边周转成本；每日构建 `High - Low` 波动率桶的 HML 策略收益。
    *   **03_liquidity_attenuation.ipynb**：利用各品种信号日 $s$ 的交易量（volume）、持仓量（open_interest）与成交额（turnover）计算 252 日滚动 z-score 与滚动分位数，合成 `activity_score`，在信号日分类为 `Low / Mid / High` 活跃度桶；与 HML 策略收益对齐后，进行双重分桶收益分析；并采用 OLS 估计包含交互项的衰减回归模型（自变量为波动率桶、活跃度得分及两者交互项），计算 HC1 异方差稳健标准误。
    *   **04_post_selection_decay.ipynb**：生成覆盖 MA、TSMOM、BREAKOUT、REVERSAL 四大类族的 26 个候选策略；以完整收益实现窗口隔离样本边界（避免跨样本信息泄露），切分为样本内（IS: 2016-01-05 至 2020-12-31）、样本外（OOS: 2021-01-01 至 2023-12-31）与伪实盘（Pseudo-Live: 2024-01-01 起）区间；利用 IS 净收益率筛选出 Top 10 策略并评估其跨区间的表现衰减分类（如 Robust, Partial Decay, Severe Decay, Decayed to Negative）。
*   **数据实证结论**：
...
*   **框架与流程**：
    *   **模型拟合与选择**（`Traditional_Model_Selection_Sugar.ipynb`）：针对 20 个主力商品期货日收益率，拟合包含常数均值或 AR(1) 均值方程，结合 GARCH(1,1)、EGARCH(1,1)、EGARCH(2,1)、GJR-GARCH(1,1)、TARCH(1,1) 条件方差方程，以及 Normal、Student-t、Skewed Student-t、GED 误差分布的 19 种参数组合；通过标准化残差 Ljung-Box 自相关检验（$z$ 与 $z^2$ 的 10/20/30 阶滞后）与 ARCH-LM 异方差检验（10阶），在通过诊断且收敛的模型中，以 BIC 最小化规则确立最优模型。
    *   **Hong-Lee 广义谱检验**（`hong_lee_20_contracts.ipynb`）：基于广义特征函数残差构建稳健的谱密度函数，在固定带宽（主带宽 $p \approx 21$）与惩罚性数据驱动自适应带宽选择器下，对最优模型进行频域非线性结构异方差检验，计算 robust M 统计量及 p 值；运行 100 次以最优估计为 DGP 的蒙特卡洛有限样本 Size 仿真，采用 Wilson Score 方法计算其 95% 置信区间。
    *   **风险预算与仓位控制**（`06_` 及风控代码）：结合最优估计与厚尾误差分布，计算单日 95% 与 99% 双向 VaR，以 Kupiec POF 回测验证其覆盖率；基于目标波动率计算各品种的仓位缩放参考系数（`Raw Scale`），当品种被谱检验拒绝（`REJECT`）或因自由度估计值 $\nu \le 4$ 导致第四矩理论不收敛时，自动乘以降权折扣系数（`Health Multiplier`），形成受统计约束的风险调节参考。
*   **数据实证结论**：
