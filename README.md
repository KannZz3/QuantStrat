...
本项目是 **QuantStrat** 与**伊利诺伊大学金融工程硕士项目（UIUC MSFE）** 2026年夏季的量化金融研究合作项目。项目采用学术规范与科学研究范式，以文献理论为锚，针对加密货币与中国商品期货主力合约，完成了一套包含**基本面锚定、行为金融异象、高阶异方差频域检验以及动态风险管理**的量化研究系统。
...
### 1. Crypto_Pricing —— 加密货币多维定价与下沿估值
*   **研究背景**：复刻与拓展 **Bhambhwani et al. (2019)**（链上基本面与网络价值锚）、**Biais et al. (2023)**（均衡交易便利收益与成本折价）以及 **Liu & Tsyvinski (2021)**（动量与注意力溢价）三篇顶尖学术文献，探索加密资产在缺乏传统贴现模型时的合理下沿估值区间。
*   **框架与流程**：
...
*   **核心结论与成果**：
    *   构建了基于 Python 的数据交叉验证系统 (`validator.py`)，实现 lead/lag 延迟相关性检验（Spearman Correlation）以及注意力指标的双源检验。
...
*   **研究背景**：复刻与拓展 **Han et al. (2013)**（高波动技术分析横截面超额收益）、**Chordia et al. (2014)**（流动性/活跃度对资本市场异象的削弱）以及 **McLean & Pontiff (2016)**（学术发表及策略筛选后收益预测性衰减）的核心假说，测试商品期货中的技术异象稳定性。
*   **框架与流程**：
...
*   **实证结论（基于 27 个主力商品期货日频 CSV 运行结果）**：
    ...
    *   **活跃度对异象呈现方向性弱衰减**：24 个活跃度衰减测试中 18 个表现出衰减方向（如 MA120 HML 收益在低活跃度下为 1.20%，高活跃度下为 -16.44%，相差 -22.24%），但统计显著性较弱（t = -1.47）。
...
*   **研究背景**：复刻与拓展 **Hong & Lee (2017)** 的广义谱波动率模型充分性检验。测试将原论文单合约设计推广至商品期货多合约时的有限样本统计特征及动态风控逻辑。
*   **框架与流程**：
    *   对 20 个被测品种日收益率拟合包含 GARCH, EGARCH, TARCH, GJR-GARCH 的 19 种参数组合，以 BIC 最小化规则选出最优模型。
